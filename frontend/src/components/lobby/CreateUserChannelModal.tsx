      ...
      <div className="glass-panel p-5 w-full max-w-sm mx-4 animate-in zoom-in-95 fade-in duration-200">
        ...
        <div className="space-y-3">
          <label>
            ...
            <input ref={inputRef} value={name} onChange={(e) => { setName(e.target.value); setError(''); }} maxLength={maxNameLen} placeholder="输入频道名" className="w-full bg-gray-800/60 border border-gray-600/50 rounded-lg px-3 py-2 mt-1 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-primary-500/50" />
          </label>
          ...
          <label>
            ...
            <input type="text" value={pwd} onChange={(e) => setPwd(e.target.value)} placeholder="4-16位（可选）" maxLength={16} className="w-full bg-gray-800/60 border border-gray-600/50 rounded-lg px-3 py-2 mt-1 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-primary-500/50" />
          </label>
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button onClick={handleSubmit} disabled={loading || !name.trim()} className="flex-1 bg-primary-600 hover:bg-primary-500 disabled:bg-gray-700 disabled:text-gray-500 text-white text-sm py-2.5 rounded-xl">{loading ? '创建中...' : '创建'}</button>
          </div>
        </div>
      </div>
    </div>
